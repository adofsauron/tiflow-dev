...
func TestCancelHandle(t *testing.T) {
	...
	err := failpoint.Enable("sdbflow/pkg/workerpool/addEventDelayPoint", "1*sleep(500)")
	require.Nil(t, err)
	defer func() {
		_ = failpoint.Disable("sdbflow/pkg/workerpool/addEventDelayPoint")
	}()
...
func TestCancelTimer(t *testing.T) {
	...
	err := failpoint.Enable("sdbflow/pkg/workerpool/unregisterDelayPoint", "sleep(5000)")
	require.Nil(t, err)
	defer func() {
		_ = failpoint.Disable("sdbflow/pkg/workerpool/unregisterDelayPoint")
	}()
...
// go test -benchmem -run='^$' -bench '^(BenchmarkWorkerpool)$' sdbflow/pkg/workerpool
