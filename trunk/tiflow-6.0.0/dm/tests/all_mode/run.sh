...
function test_query_timeout() {
	...
	export GO_FAILPOINTS="sdbflow/dm/syncer/BlockSyncStatus=return(\"5s\")"
	...
	# there's only 2 rows in result, one for SDM-worker's source-level status, one for SHOW PROCESSLIST
...
function test_stop_task_before_checkpoint() {
	...
	export GO_FAILPOINTS='sdbflow/dm/loader/WaitLoaderStopAfterInitCheckpoint=return(5)'
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
	run_dm_worker $WORK_DIR/worker2 $WORKER2_PORT $cur/conf/dm-worker2.toml
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER2_PORT
	...
	export GO_FAILPOINTS='sdbflow/dm/loader/WaitLoaderStopBeforeLoadCheckpoint=return(5)'
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
	run_dm_worker $WORK_DIR/worker2 $WORKER2_PORT $cur/conf/dm-worker2.toml
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER1_PORT
	check_rpc_alive $cur/../bin/check_worker_online 127.0.0.1:$WORKER2_PORT
...
function test_fail_job_between_event() {
	...
	inject_points=(
		"sdbflow/dm/dm/worker/TaskCheckInterval=return(\"500ms\")"
		"sdbflow/dm/syncer/countJobFromOneEvent=return()"
		"sdbflow/dm/syncer/flushFirstJob=return()"
		"sdbflow/dm/syncer/failSecondJob=return()"
	)
	...
	inject_points=(
		"sdbflow/dm/dm/worker/TaskCheckInterval=return(\"500ms\")"
		"sdbflow/dm/syncer/countJobFromOneGTID=return()"
		"sdbflow/dm/syncer/flushFirstJob=return()"
		"sdbflow/dm/syncer/failSecondJob=return()"
	)
	export GO_FAILPOINTS="$(join_string \; ${inject_points[@]})"
