...
func (t *testSchedulerSuite) TestTransferSource() {
	...
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/failToReplaceSourceBound", `return()`))
	require.Error(t.T(), s.TransferSource(ctx, sourceID1, workerName1))
	require.Equal(t.T(), worker4, s.bounds[sourceID1])
	require.Equal(t.T(), WorkerFree, worker1.Stage())
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/failToReplaceSourceBound"))
	...
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus", `return("notInSyncUnit")`))
	defer failpoint.Disable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus") //nolint:errcheck
	require.True(t.T(), terror.ErrSchedulerRequireRunningTaskInSyncUnit.Equal(s.TransferSource(ctx, sourceID1, workerName1)))
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus"))

	// test can't transfer when query status met error
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus", `return("error")`))
	...
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/skipBatchOperateTaskOnWorkerSleep", `return()`))
	defer failpoint.Disable("sdbflow/dm/dm/master/scheduler/skipBatchOperateTaskOnWorkerSleep") //nolint:errcheck
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus", `return("allTaskIsPaused")`))

	// we only retry 10 times, open a failpoint to make need retry more than 10 times, so this transfer will fail
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/batchOperateTaskOnWorkerMustRetry", `return(11)`))
	require.True(t.T(), terror.ErrSchedulerPauseTaskForTransferSource.Equal(s.TransferSource(ctx, sourceID1, workerName1)))
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/batchOperateTaskOnWorkerMustRetry"))

	// now we can transfer successfully after 2 times retry
	s.expectSubTaskStages.Store("test", map[string]ha.Stage{sourceID1: {Expect: pb.Stage_Running}})
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/batchOperateTaskOnWorkerMustRetry", `return(2)`))
	require.NoError(t.T(), s.TransferSource(ctx, sourceID1, workerName1))
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/batchOperateTaskOnWorkerMustRetry"))
	require.Equal(t.T(), worker1, s.bounds[sourceID1])
	require.Equal(t.T(), WorkerBound, worker1.Stage())
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/operateWorkerQueryStatus"))
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/skipBatchOperateTaskOnWorkerSleep"))
}
...
func (t *testSchedulerSuite) TestCloseAllWorkers() {
	...
	require.NoError(t.T(), failpoint.Enable("sdbflow/dm/dm/master/scheduler/failToRecoverWorkersBounds", "return"))
	...
	require.NoError(t.T(), failpoint.Disable("sdbflow/dm/dm/master/scheduler/failToRecoverWorkersBounds"))
...
func (t *testSchedulerSuite) TestUpdateSubTasksAndSourceCfg() {
	...
	t.NoError(failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateCheckSubtasksCanUpdate", `return("error")`))
	t.Regexp("query error", s.UpdateSubTasks(ctx, subtaskCfg1))
	t.NoError(failpoint.Disable("sdbflow/dm/dm/master/scheduler/operateCheckSubtasksCanUpdate"))

	// can't updated when worker rpc check not pass
	t.NoError(failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateCheckSubtasksCanUpdate", `return("failed")`))
	t.True(terror.ErrSchedulerSubTaskCfgUpdate.Equal(s.UpdateSubTasks(ctx, subtaskCfg1)))
	t.NoError(failpoint.Disable("sdbflow/dm/dm/master/scheduler/operateCheckSubtasksCanUpdate"))

	// update success
	subtaskCfg1.Batch = 1000
	t.NoError(failpoint.Enable("sdbflow/dm/dm/master/scheduler/operateCheckSubtasksCanUpdate", `return("success")`))
