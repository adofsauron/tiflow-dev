...
function safe_mode_duration() {
	...
	export GO_FAILPOINTS="sdbflow/dm/syncer/SkipSaveGlobalPoint=return()"
	run_dm_master $WORK_DIR/master $MASTER_PORT $cur/conf/dm-master.toml
	check_rpc_alive $cur/../bin/check_master_online 127.0.0.1:$MASTER_PORT
	# worker1 -> source1
	run_dm_worker $WORK_DIR/worker1 $WORKER1_PORT $cur/conf/dm-worker1.toml
...
function run() {
	...
	export GO_FAILPOINTS="sdbflow/dm/syncer/ShardSyncedExecutionExit=return(true);sdbflow/dm/syncer/SafeModeInitPhaseSeconds=return(\"300s\")"
