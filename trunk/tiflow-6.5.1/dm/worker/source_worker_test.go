...
func (t *testServer2) SetUpSuite(c *C) {
	...
	c.Assert(failpoint.Enable("sdbflow/dm/worker/MockGetSourceCfgFromETCD", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("sdbflow/dm/worker/SkipRefreshFromETCDInUT", `return()`), IsNil)
}

func (t *testServer2) TearDownSuite(c *C) {
	getMinLocForSubTaskFunc = getMinLocForSubTask
	c.Assert(failpoint.Disable("sdbflow/dm/worker/MockGetSourceCfgFromETCD"), IsNil)
	c.Assert(failpoint.Disable("sdbflow/dm/worker/SkipRefreshFromETCDInUT"), IsNil)
}
...
func (t *testServer2) TestTaskAutoResume(c *C) {
	...
	c.Assert(failpoint.Enable("sdbflow/dm/worker/mockCreateUnitsDumpOnly", `return(true)`), IsNil)
	//nolint:errcheck
	defer failpoint.Disable("sdbflow/dm/worker/mockCreateUnitsDumpOnly")
